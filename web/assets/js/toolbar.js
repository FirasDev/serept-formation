Sfjs = (function () {
    ...
    var requestStack = [];
    var extractHeaders = function (xhr, stackElement) {            /* Here we avoid to call xhr.getResponseHeader in order to */
    ...
    if (window.fetch && window.fetch.polyfill === undefined) {
        ...
        window.fetch = function () {
            ...
            if (!url.match(new RegExp("^\/(app(_[\\w]+)?\\.php\/)?_wdt"))) {
                var method = 'GET';
    ...
    if (window.XMLHttpRequest && XMLHttpRequest.prototype.addEventListener) {
        ...
        XMLHttpRequest.prototype.open = function (method, url, async, user, pass) {
            ...
            } else if (0 === url.indexOf('http\x3A\x2F\x2F127.0.0.1\x3A8000')) {
                path = url.substr(21);
            }
            if (!path.match(new RegExp("^\/(app(_[\\w]+)?\\.php\/)?_wdt"))) {
                var stackElement = {error: false, url: url, method: method, type: 'xhr', start: new Date()};
                var idx = requestStack.push(stackElement) - 1;
    ...
    return {
        ...
        toggle: function (selector, elOn, elOff) {
            var tmp = elOn.style.display, el = document.getElementById(selector);
        ...
        createTabs: function () {
            ...
            for (var i = 0; i < tabGroups.length; i++) {
                ...
                for (var j = 0; j < tabs.length; j++) {
                    ...
                    if (j == 0) {
                        addClass(tabNavigationItem, 'active');
                    }
